...
class Builder():
    ...
    def build_version(self, token):
        ...
        if "ERROR" in set_items:
            index = set_items.index("ERROR")
            not_set_item = set_items[index + 1]
            set_items[:] = []
            text_file = open("warnings.tmp", "a")
            text_file.write(
                u"You run %s in version %s without setting metrics for the device.\n" % (not_set_item, token[2]))
            text_file.close()
        #  raise ConfigurationParserException('Invalid syntax.', \
        #  syntax_errors=[u"You must set %s in version %s before you run it." % (not_set_item, token[2])])
        set_items[:] = []
        return v
    ...
    def build_run_host(self, token):
        ...
        if token[3] not in set_items:
            set_items.append("ERROR")
            set_items.append(token[3])

        if len(token) == 8:

            if isinstance(token[5],
                          int):  # RUN HOST IDENTIFIER version_channels version_repetition BLOCKOPEN BLOCKCLOSE
            ...
            if isinstance(token[6],
                          list):  # RUN HOST IDENTIFIER version_channels version_repetition version_repetition_channels BLOCKOPEN BLOCKCLOSE
                run_host.repetitions = token[5]
...
class ConfigParserExtension(LexYaccParserExtension):
    ...
    def _extend(self):
        ...
        self.parser.add_token('error', func=self.token_error, states=['versionsrepeatedchannels'],
                              include_in_tokens=False)
        self.parser.add_token('ignore', "\t ", states=['versionsrepeatedchannels'], include_in_tokens=False)
        self.parser.add_token('newline', func=self.t_newline, states=['versionsrepeatedchannels'],
                              include_in_tokens=False)
