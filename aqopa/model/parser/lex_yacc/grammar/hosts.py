...
class ModelParserExtension(LexYaccParserExtension):
    ...
    def hosts_specification(self, t):
        ...
        """
        if not self.parser.store.functions:
            raise ModelParserException('Invalid syntax.',
                                       syntax_errors=[u"You do not have any functions in model."])
        if not self.parser.store.channels:
            raise ModelParserException('Invalid syntax.',
                                       syntax_errors=[u"You do not have any channels in model."])
        pass
    ...
    def host(self, t):
        ...
        host : HOST IDENTIFIER LPARAN host_schedule_algorithm RPARAN host_channels BLOCKOPEN host_body BLOCKCLOSE
            | HOST IDENTIFIER LPARAN host_schedule_algorithm RPARAN host_channels BLOCKOPEN host_predefined_values host_body BLOCKCLOSE
        """
        global communication_instruction

        if type(t[6]) != list and t[6] != '*' and communication_instruction:
            communication_instruction = False
            raise ModelParserException('Invalid syntax.',
                                       syntax_errors=[u"You have to add some channels in host %s"
                                                      " because you have communication instruction there. This host ends in line %s" % (
                                                      t[2], t.lexer.lineno - 1)])
        communication_instruction = False
    ...
    def host_process(self, t):
        ...
        """
        global communication_instruction
        no_channels_process = False
        if type(t[3]) != list and str(t[3]) != '*':
            no_channels_process = True
        for x in t:
            if type(x) == list:
                for y in x:
                    if str(y).split() > 0 and str(y).split()[0] == '<aqopa.model.CommunicationInstruction':
                        communication_instruction = True
                        if no_channels_process:
                            raise ModelParserException('Invalid syntax.',
                                                       syntax_errors=[u"You have to add some channels in %s %s"
                                                                      " because you have communication instruction there. This process ends in line %s" % (
                                                                      t[1], t[2], t.lexer.lineno - 1)])
